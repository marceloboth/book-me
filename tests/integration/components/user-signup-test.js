...
test('it invokes passed `registerUser` action when clicking on signup button', (assert) => {
  const {
    $,
  } = this;
...
test('it does not invoke passed `registerUser` action when there is a validation error and displays the error messages', (assert) => {
  const {
    $,
  } = this;
